...
def get_available_time_slots(phlebotomist_id, appointment_date, duration_minutes=30):
    ...
    logger.info(f"Phlebotomist {phlebotomist_id} on {appointment_date}: {len(available_slots)} slots available")
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                messages.success(request, f"Welcome, {first_name}! Let's set up your account.")
                return redirect('donor:choose-username')
...
def needs_eligibility_check(user):
    """
    Returns False for new donors - they can access dashboard with awareness modal.
    """
    try:
        donor = Donor.objects.get(user=user)
    except Donor.DoesNotExist:
        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    ...
                    send_eligibility_notification(donor, is_eligible, reasons)
            ...
            messages.error(
                request, 
                f"Please correct the {error_count} error(s) highlighted below."
            )
    else:
...
def send_eligibility_notification(donor, is_eligible, reasons=None):
    """Send notification about eligibility status"""
    
    if is_eligible:
        title = "You're Eligible to Donate Blood!"
        message = (
            "Great news! Based on your responses, you are eligible to donate blood. "
            "You can now schedule your donation appointment."
        )
    else:
        title = "Your Eligibility Status"
        message = (
            "Thank you for completing the eligibility form. Based on your responses, "
            "you may have temporary deferrals. Please check your eligibility status "
            "for more details."
        )
        if reasons:
...
def donate_blood_view(request):
    ...
    try:
        ...
        messages.error(request, "You must complete your donor profile before donating blood.")
        return redirect('donor:donor-profile')
    ...
    if not is_profile_complete:
        messages.error(
            request, 
            f"Please complete your donor profile before scheduling a donation. Missing: {', '.join(missing_fields)}"
        )
        return redirect('donor:donor-profile')
    ...
    try:
        ...
        messages.info(request, "Please complete your eligibility form before donating blood.")
        return redirect('donor:donor-eligibility')

    if not eligibility.approved:
        messages.warning(request, "Your eligibility has not been approved yet.")
        return redirect('donor:donor-eligibility-status')
        ...
        if donate_form.is_valid():
            try:
                ...
                try:
                    ...
                    # Send notifications
                    send_donation_notifications(donation, appointment, donor, phlebotomist)
                    ...
                    logger.error(f"Failed to create appointment for donation {donation.id}: {str(e)}")
                    donation.delete()
...
def send_donation_notifications(donation, appointment, donor, phlebotomist):
    """Send notifications for a new donation appointment"""
    ...
    NotificationService.send_to_user(
        ...
        message=(
            f"Your blood donation appointment has been scheduled for "
            f"{appointment.date.strftime('%B %d, %Y at %I:%M %p')} at "
            f"{appointment.center.name}. Please arrive 15 minutes early."
        )
    )
    
    NotificationService.send_to_user(
        recipient_user=phlebotomist.user,
        sender_user=donor.user,
        title="New Donation Appointment",
        message=(
            f"New donation appointment scheduled with {donor.user.get_full_name()} "
            f"on {appointment.date.strftime('%B %d, %Y at %I:%M %p')}."
        )
    )
    
    if hasattr(settings, 'NOTIFY_ADMINS_ON_DONATION') and settings.NOTIFY_ADMINS_ON_DONATION:
        NotificationService.send_to_all_admins(
            title="New Donation Scheduled",
            message=(
                f"A new donation has been scheduled by {donor.user.get_full_name()} "
                f"at {appointment.center.name} on {appointment.date.strftime('%B %d, %Y')}."
            )
        )
...
def cancel_donation_request_view(request, donation_id):
    ...
    if donation.status.lower() in ['pending', 'approved']:
        ...
        donation.cancelled_by = 'donor'
        donation.cancelled_at = now
        donation.save(update_fields=['status', 'cancelled_by', 'cancelled_at'])
    ...
    if appointment.status.lower() in ['pending', 'approved']:
        ...
        send_cancellation_notifications(donation, appointment, user.donor)

    messages.success(request, "Your donation request and appointment have been cancelled successfully.")
    return redirect('donation-history')

def send_cancellation_notifications(donation, appointment, donor):
    ...
    NotificationService.send_to_user(
        ...
        message=(
            f"Your donation appointment for {appointment.date.strftime('%B %d, %Y at %I:%M %p')} "
    ...
    if appointment.phlebotomist:
        NotificationService.send_to_user(
            recipient_user=appointment.phlebotomist.user,
            sender_user=donor.user,
            title="Donation Appointment Cancelled",
            message=(
                f"Donor {donor.user.get_full_name()} has cancelled their appointment "
                f"scheduled for {appointment.date.strftime('%B %d, %Y at %I:%M %p')}."
            )
        )
...
# ==========================================
# NOTIFICATION REDIRECT VIEWS
# ==========================================

@login_required(login_url='donorlogin')
@username_required
def donor_notifications_view(request):
    """Redirect to universal notifications"""
...
def mark_notification_read(request, pk):
    """Redirect to universal mark read"""
