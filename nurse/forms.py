...
class NurseSignupForm(forms.ModelForm):
    ...
    def clean_phone(self):
        ...
        if Nurse.objects.filter(phone=phone_clean).exists():
    ...
    def __init__(self, *args, **kwargs):
        ...
        if center:
            self.fields['nurse'].queryset = Nurse.objects.filter(
                ...
                is_approved=True  # Only show approved nurses
            ...
            self.fields['nurse'].queryset = self.instance.donation_center.nurses.filter(
                is_approved=True
            ).order_by('user__first_name')
